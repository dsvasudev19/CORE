...
const ClientLayout = () => {
    ...
    const navigation = [
        ...
        { name: 'Support', href: '/c/support', icon: HelpCircle },
        { name: 'Settings', href: '/c/settings', icon: Settings },
    ];

    const notifications = [
        { id: 1, title: 'Invoice Ready', message: 'Invoice #INV-2024-045 is ready', time: '10m ago', unread: true },
        { id: 2, title: 'Project Update', message: 'Website redesign milestone completed', time: '2h ago', unread: true },
        { id: 3, title: 'New Message', message: 'Response from project manager', time: '5h ago', unread: false },
    ];
    ...
    return (
        <div className="min-h-screen bg-steel-50 flex">
            {/* ────────────────────── LEFT SIDEBAR ────────────────────── */}
            <aside
                className={`fixed left-0 top-0 h-screen bg-white border-r border-steel-200 z-40 transition-all duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
            ...
            >
                <div className="flex flex-col h-full relative">
                    ...
                    >
                        {sidebarCollapsed ? (
                            <ChevronRight size={14} className="text-steel-600" />
                        ) : (
                            <ChevronLeft size={14} className="text-steel-600" />
                        )}
                    </button>
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                            ...
                            {!sidebarCollapsed && (
                                <div className="min-w-0 transition-opacity duration-200">
                                    <h1 className="text-lg font-bold text-steel-900 tracking-tight">Client Portal</h1>
                                    <p className="text-xs text-steel-500">TechCorp Inc.</p>
                                </div>
                            )}
                        </div>
                        <button
                        ...
                        {navigation.map((item) => {
                            ...
                            return (
                                ...
                                >
                                    <Icon
                                        size={18}
                                        className={`transition-colors duration-200 ${active ? 'text-burgundy-600' : ''}`}
                                    />
                    ...
                    <div className="p-3 border-t border-steel-200">
                        <div
                            className={`flex items-center gap-3 p-3 bg-gradient-to-br from-steel-50 to-steel-100 rounded-lg transition-all duration-200 ${sidebarCollapsed ? 'justify-center' : ''
                                }`}
                        >
                            <div className="w-10 h-10 bg-gradient-to-br from-burgundy-100 to-blue-200 rounded-full flex items-center justify-center flex-shrink-0">
                ...
                <header className="bg-white border-b-2 border-gray-300 sticky top-0 z-30 shadow-sm">
                    <div className="flex items-center justify-between h-16 px-4 sm:px-6">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <Link
                                to="/c/support"
                                className="hidden lg:flex items-center gap-2.5 px-4 py-2.5 rounded-full bg-gradient-to-r from-burgundy-50 to-rose-50 border-2 border-burgundy-200 hover:border-burgundy-400 hover:from-burgundy-100 hover:to-rose-100 transition-all shadow-md hover:shadow-lg group"
                            >
                                <HelpCircle size={18} strokeWidth={2.5} className="text-burgundy-600 group-hover:text-burgundy-700 transition-colors" />
                                <span className="text-sm font-semibold text-burgundy-700 group-hover:text-burgundy-800 transition-colors">
                                    Get Support
                                </span>
                            </Link>
                                    ...
                                    <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-xl border-2 border-gray-300 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                                        ...
                                        <div className="max-h-96 overflow-y-auto">
                                            {notifications.map((n) => (
                                                <div
                                                    key={n.id}
                                                    className={`px-4 py-3 hover:bg-gray-50 cursor-pointer transition-colors border-b border-gray-200 ${n.unread ? 'bg-burgundy-50/30' : ''
                                                        }`}
                                                >
                                        ...
                                        <div className="px-4 py-2 border-t-2 border-gray-300 bg-gray-50">
                                            <Link
                                                to="/c/notifications"
                                                className="text-sm font-medium text-burgundy-600 hover:text-burgundy-700 transition-colors"
                                            >
                            ...
                            <div className="relative" ref={userMenuRef}>
                                ...
                                >
                                    <div className="w-9 h-9 bg-gradient-to-br from-burgundy-100 to-blue-200 rounded-full flex items-center justify-center shadow-sm border-2 border-burgundy-300">
