...
public class ProjectPhaseValidator {
    ...
    public void validateBeforeCreate(ProjectPhaseDTO dto) {
        if (dto == null)
            throw new ValidationFailedException("error.phase.null",null);

        if (dto.getProjectId() == null)
            throw new ValidationFailedException("error.phase.projectid.required",null);

        if (!StringUtils.hasText(dto.getName()))
            throw new ValidationFailedException("error.phase.name.required",null);

        if (dto.getProgressPercentage() != null &&
                (dto.getProgressPercentage() < 0 || dto.getProgressPercentage() > 100))
            throw new ValidationFailedException("error.phase.progress.invalid",null);

        if (dto.getStartDate() != null && dto.getEndDate() != null &&
                dto.getStartDate().isAfter(dto.getEndDate()))
            throw new ValidationFailedException("error.phase.dates.invalid",null);
    }

    public void validateBeforeUpdate(Long id, ProjectPhaseDTO dto) {
        if (id == null)
            throw new ValidationFailedException("error.phase.id.required",null);
    ...
    public void validateBeforeReorder(Long projectId, List<Long> orderedPhaseIds) {
        ...
        if (orderedPhaseIds == null || orderedPhaseIds.isEmpty())
            throw new ValidationFailedException("error.phase.reorder.list.empty",null);
    }
}
