...
const ClientDetailsPage: React.FC = () => {
  ...
  const fetchClient = async () => {
    ...
    setError('');
    try {
      const data = await clientService.getClientDetailed(Number(id));
      setClient(data);
    } catch {
  ...
  const handleStatusToggle = async () => {
    try {
      if (client.status === 'ACTIVE') {
        await clientService.deactivateClient(Number(id));
        setClient({ ...client, status: 'INACTIVE' });
      } else {
        await clientService.activateClient(Number(id));
  ...
  const handleUploadDocument = async () => {
    ...
    try {
      const newDoc = await clientDocumentService.uploadDocument(Number(id), {
        file: docForm.file,
        title: docForm.title,
  ...
  const handleDeleteDocument = async (docId: number) => {
    ...
    try {
      await clientDocumentService.deleteDocument(Number(id), docId);
      setClient({
        ...client,
  ...
  const handleAddRepresentative = async () => {
    ...
    try {
      ...
      const newRep = await clientRepresentativeService.addRepresentative({
        clientId: Number(id),
      ...
      setClient({
        ...client,
        representatives: [...client.representatives, newRep]
      });
  ...
  const handleRemoveRepresentative = async (repId: number) => {
    ...
    try {
      await clientRepresentativeService.deactivateRepresentative(repId);
      setClient({
        ...client,
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium ${client.status === 'ACTIVE'
