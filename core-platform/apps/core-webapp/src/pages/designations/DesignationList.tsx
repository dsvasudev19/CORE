...
const DesignationList: React.FC = () => {
  ...
  const [editDesig, setEditDesig] = useState<DesignationDTO | null>(null);
  ...
  const fetchDesignations = async () => {
    ...
    setLoading(true);
    try {
      const data = await designationService.getAllDesignations(user.organizationId);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (editDesig) {
      designationService
        .updateDesignation(editDesig.id, { ...form })
        .then(() => {
          fetchDesignations();
          setShowModal(false);
      ...
      designationService
        .createDesignation({ ...form, organizationId: user.organizationId })
        .then(() => {
          fetchDesignations();
          setShowModal(false);
