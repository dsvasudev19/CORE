...
public class TimeLogController {
    ...
    public ResponseEntity<?> getAllOrganizationTimeLogs(
            @PathVariable Long organizationId,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fromDate,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate toDate
    ) {
        log.info("📊 Fetching all time logs for organization {}", organizationId);
        return helper.success("Organization time logs fetched",
                timeLogService.getAllOrganizationTimeLogs(organizationId, fromDate, toDate));
    }
    
    @GetMapping("/organization/{organizationId}/statistics")
    public ResponseEntity<?> getOrganizationTimeStatistics(
            @PathVariable Long organizationId,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fromDate,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate toDate
    ) {
        log.info("📈 Fetching time statistics for organization {}", organizationId);
        return helper.success("Organization time statistics fetched",
                timeLogService.getOrganizationTimeStatistics(organizationId, fromDate, toDate));
    }
    
    @GetMapping("/organization/{organizationId}/employee-summary")
    public ResponseEntity<?> getEmployeeTimeSummary(
            @PathVariable Long organizationId,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fromDate,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate toDate
    ) {
